    ...
    M_operation(operation)
{
    ...
    bool valid = false;
    if (M_lhs->type() == any_pointer::get_tag<std::int64_t>()) valid = true;
    if (M_lhs->type() == any_pointer::get_tag<double>()) {
        if (M_operation != Modulo) {
            valid = true;
        }
    }
    if (M_lhs->type() == any_pointer::get_tag<std::string>()) {
        if (M_operation == Plus) {
            valid = true;
        }
    }
