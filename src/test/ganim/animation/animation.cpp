...
TEST_CASE("Animation basics", "[animation]") {
    auto test = ObjectPtr<TestAnimatable>();
...
TEST_CASE("Animation with rvalues", "[animation]") {
    ...
    // waits a frame so we don't check on_delete here, and if it was deleted
    // already we can't access test2.updated anymore
    scene.frame_advance();
    REQUIRE(on_delete == 1);
}

TEST_CASE("Animatable different framerates", "[animation]") {
    auto test1 = ObjectPtr<TestAnimatable>();
    auto test2 = ObjectPtr<TestAnimatable>();
...
TEST_CASE("Animating rvalues", "[animation]")
{
    ...
    auto scene = TestScene(1, 1, 1, 1, 1);
    scene.add(test);
    animate(scene, make_group(test)).set_color(Color("FF0000"));
...
TEST_CASE("Animating twice with groups", "[animation]") {
    ...
    auto scene = TestScene(1, 1, 1, 1, 1);
    animate(scene, test1);
    REQUIRE_THROWS(animate(scene, make_group(test1)));
    animate(scene, make_group(test2));
