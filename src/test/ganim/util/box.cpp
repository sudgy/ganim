...
TEST_CASE("merge_boxes", "[object]") {
    ...
    const auto res1p1 = pga3_to_vga3(res1.get_inner_lower_left_vertex());
    const auto res1p2 = pga3_to_vga3(res1.get_outer_upper_right_vertex());
    const auto res2p1 = pga3_to_vga3(res2.get_inner_lower_left_vertex());
    const auto res2p2 = pga3_to_vga3(res2.get_outer_upper_right_vertex());
    const auto res3p1 = pga3_to_vga3(res3.get_inner_lower_left_vertex());
    const auto res3p2 = pga3_to_vga3(res3.get_outer_upper_right_vertex());
    const auto res4p1 = pga3_to_vga3(res4.get_inner_lower_left_vertex());
    const auto res4p2 = pga3_to_vga3(res4.get_outer_upper_right_vertex());
    const auto res5p1 = pga3_to_vga3(res5.get_inner_lower_left_vertex());
    const auto res5p2 = pga3_to_vga3(res5.get_outer_upper_right_vertex());
...
TEST_CASE("transform_box", "[object]") {
    ...
    const auto res5p1 = pga3_to_vga3(res5.get_inner_lower_left_vertex());
    const auto res5p2 = pga3_to_vga3(res5.get_outer_upper_right_vertex());
    const auto res6p1 = pga3_to_vga3(res6.get_inner_lower_left_vertex());
    const auto res6p2 = pga3_to_vga3(res6.get_outer_upper_right_vertex());
    const auto res7p1 = pga3_to_vga3(res7.get_inner_lower_left_vertex());
    const auto res7p2 = pga3_to_vga3(res7.get_outer_upper_right_vertex());
    const auto res8p1 = pga3_to_vga3(res8.get_inner_lower_left_vertex());
    const auto res8p2 = pga3_to_vga3(res8.get_outer_upper_right_vertex());
    const auto res9p1 = pga3_to_vga3(res9.get_inner_lower_left_vertex());
    const auto res9p2 = pga3_to_vga3(res9.get_outer_upper_right_vertex());
...
TEST_CASE("Box sides", "[object]") {
...

    {
        ...
        REQUIRE_THAT(box.get_center_3d(),
                GAEquals(!(e0 + 2.5*e1 + 3.5*e2 + 4.5*e3)));
        REQUIRE_THAT(box.get_left_3d(),
                GAEquals(!(e0 + e1 + 3.5*e2 + 4.5*e3)));
        REQUIRE_THAT(box.get_right_3d(),
                GAEquals(!(e0 + 4*e1 + 3.5*e2 + 4.5*e3)));
        REQUIRE_THAT(box.get_up_3d(),
                GAEquals(!(e0 + 2.5*e1 + 5*e2 + 4.5*e3)));
        REQUIRE_THAT(box.get_down_3d(),
                GAEquals(!(e0 + 2.5*e1 + 2*e2 + 4.5*e3)));
        REQUIRE_THAT(box.get_out_3d(),
                GAEquals(!(e0 + 2.5*e1 + 3.5*e2 + 6*e3)));
        REQUIRE_THAT(box.get_in_3d(),
                GAEquals(!(e0 + 2.5*e1 + 3.5*e2 + 3*e3)));
        REQUIRE_THAT(box.get_outer_upper_left_vertex(),
                GAEquals(!(e0 + 1*e1 + 5*e2 + 6*e3)));
        REQUIRE_THAT(box.get_outer_upper_right_vertex(),
                GAEquals(!(e0 + 4*e1 + 5*e2 + 6*e3)));
        REQUIRE_THAT(box.get_outer_lower_left_vertex(),
                GAEquals(!(e0 + 1*e1 + 2*e2 + 6*e3)));
        REQUIRE_THAT(box.get_outer_lower_right_vertex(),
                GAEquals(!(e0 + 4*e1 + 2*e2 + 6*e3)));
        REQUIRE_THAT(box.get_inner_upper_left_vertex(),
                GAEquals(!(e0 + 1*e1 + 5*e2 + 3*e3)));
        REQUIRE_THAT(box.get_inner_upper_right_vertex(),
                GAEquals(!(e0 + 4*e1 + 5*e2 + 3*e3)));
        REQUIRE_THAT(box.get_inner_lower_left_vertex(),
                GAEquals(!(e0 + 1*e1 + 2*e2 + 3*e3)));
        REQUIRE_THAT(box.get_inner_lower_right_vertex(),
